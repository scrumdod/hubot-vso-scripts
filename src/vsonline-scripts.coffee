...
&redirect_uri=#{escape(oauthCallbackUrl)}"
  ...
  handleVsoError = (msg, err) ->
    msg.reply "Unable to executed command: #{util.inspect(err)}" if err
  ...
  #########################################
  robot.respond /vso create (PBI|Task|Feature|Impediment|Bug) (.+) with description (.+)/im, (msg) ->
  ...
  robot.respond /vso today/i, (msg) ->
    ...
    runVsoCmd msg, cmd: (client) ->
      ...
      getCommitsForUser 1, msg, (pushes, repo) ->
        ...
        mypushes=[]
        if numPushes > 0
          mypushes.push "Here are your commits:"
          for push in pushes
